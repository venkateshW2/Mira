    ...
    : segmentId(segmentIdIn), database(databaseIn), laf(lafIn), fileIds(std::move(fileIdsIn))
{
    ...
    addFieldRow(grooveRow, "Groove");
    addFieldRow(swingRow, "Swing");
    addFieldRow(lowEndRow, "Low end");
    addFieldRow(motionRow, "Motion");
...
FileDetailsContent::EffectiveFields FileDetailsContent::computeFields(const mira::FileRecord& rec) const
{
    ...
    if (onsets.empty())
    {
        out.grooveRef = out.swingRef =
            juce::String(juce::CharPointer_UTF8("no onset times â re-analyze with Groove on"));
    }
    else
    {
        ...
        if (!g.omittedReason.empty())
        {
            out.grooveRef = "analyzed: " + juce::String(g.grid.strength, 2)
                          + juce::String(juce::CharPointer_UTF8("Ã â too flat to call a grid"));
            out.swingRef = out.grooveRef;
        }
        else
        {
            out.grooveRef = "analyzed: " + juce::String(g.grid.strength, 2)
                          + juce::String(juce::CharPointer_UTF8("Ã grid lock at "))
