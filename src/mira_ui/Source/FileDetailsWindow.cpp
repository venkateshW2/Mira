    ...
    : segmentId(segmentIdIn), database(databaseIn), laf(lafIn), fileIds(std::move(fileIdsIn))
{
    ...
    // The bucket words CaptionFields.h derives, in the order its thresholds run.
    addFieldRow(grooveRow, "Groove", { "organic", "steady", "programmed" });
    addFieldRow(swingRow, "Swing", { "straight", "light swing", "swung" });
    addFieldRow(lowEndRow, "Low end", { "light", "balanced", "heavy" });
    addFieldRow(motionRow, "Motion", { "static", "shifting", "morphing" });
    // Read-only until meter earns a caption slot: the editor is shown so the row matches
    // its neighbours, but it takes no input because nothing would read the override.
    addFieldRow(meterRow, "Meter");
    meterRow.editor.setReadOnly(true);
    meterRow.editor.setColour(juce::TextEditor::textColourId, MiraLookAndFeel::textDim);

    generatedLabel.setText("Generated by mira", juce::dontSendNotification);
    generatedLabel.setFont(laf.sansSemiBold(14.5f));
    generatedLabel.setColour(juce::Label::textColourId, MiraLookAndFeel::text);
    addAndMakeVisible(generatedLabel);

    generatedEditor.setMultiLine(true, true);
    generatedEditor.setReadOnly(true);
    generatedEditor.setScrollbarsShown(true);
    generatedEditor.setFont(laf.monoRegular(11.5f));
    generatedEditor.setColour(juce::TextEditor::backgroundColourId, MiraLookAndFeel::surface3);
    generatedEditor.setColour(juce::TextEditor::textColourId, MiraLookAndFeel::textDim);
    generatedEditor.setColour(juce::TextEditor::outlineColourId, MiraLookAndFeel::border);
    addAndMakeVisible(generatedEditor);
                                      ...
                                      const juce::StringArray& vocabulary)
{
    ...
    // A fixed vocabulary where one exists. CAPTION-TAGGING.md already settled this for
    // the folder tags -- "Fixed lists, never free text: a LoRA learns whatever mapping it
    // is fed, so a vocabulary that drifts between folders teaches the model nothing" --
    // and the reasoning is identical here: three spellings of one idea are three tokens
    // the model must learn separately from a third of the examples each, and a word
    // invented for a single file appears once in 94 and is learned by nothing.
    //
    // Offered rather than enforced. Typing a deliberate new word is still allowed,
    // because the alternative is that the only way to record a real new distinction is
    // to not record it.
    if (!vocabulary.isEmpty())
    {
        row.hasVocab = true;
        row.vocab.addItem("-- measured --", 1);
        for (int i = 0; i < vocabulary.size(); ++i) row.vocab.addItem(vocabulary[i], i + 2);
        row.vocab.setSelectedId(1, juce::dontSendNotification);
        auto* r = &row;
        row.vocab.onChange = [r] {
            const int sel = r->vocab.getSelectedId();
            if (sel <= 1) return;
            r->editor.setText(r->vocab.getItemText(sel - 1), juce::sendNotificationSync);
            r->vocab.setSelectedId(1, juce::dontSendNotification);
        };
        addAndMakeVisible(row.vocab);
    }
}
...
FileDetailsContent::EffectiveFields FileDetailsContent::computeFields(const mira::FileRecord& rec) const
{
    ...
    // Groove/sound-design. The effective value comes from CaptionFields itself rather
    // than being re-derived here, so the row always shows exactly what the caption will
    // say -- including the omission when the onsets never locked to a grid.
    auto caption = extractCaptionFields(database, rec);
    out.groove = caption.groove ? juce::String(*caption.groove) : juce::String();
    out.swing = caption.swing ? juce::String(*caption.swing) : juce::String();
    out.lowEnd = caption.lowEnd ? juce::String(*caption.lowEnd) : juce::String();
    out.motion = caption.motion ? juce::String(*caption.motion) : juce::String();

    auto onsets = database.jsonDoubleArray(rec.machine, "$.onset_times");
    if (onsets.empty())
    {
        out.grooveRef = out.swingRef =
            juce::String(juce::CharPointer_UTF8("no onset times — re-analyze with Groove on"));
    }
    else
    {
        auto g = mira::analyzeGroove(onsets,
                                     database.jsonExtractDouble(rec.machine, "$.rhythm.essentia_bpm"),
                                     database.jsonExtractDouble(rec.machine, "$.rhythm.beat_this_bpm"));
        if (!g.omittedReason.empty())
        {
            out.grooveRef = "analyzed: " + juce::String(g.grid.strength, 2)
                          + juce::String(juce::CharPointer_UTF8("× — too flat to call a grid"));
            out.swingRef = out.grooveRef;
        }
        else
        {
            out.grooveRef = "analyzed: " + juce::String(g.grid.strength, 2)
                          + juce::String(juce::CharPointer_UTF8("× grid lock at "))
                          + juce::String(g.grid.bpm, 1) + " bpm";
            out.swingRef = g.swing ? "analyzed: " + juce::String(*g.swing * 100.0, 1)
                                         + "% of the beat (50% = straight)"
                                   : juce::String("analyzed: not measurable");
        }
    }

    auto subRatio = database.jsonExtractDouble(rec.machine, "$.dsp.sub_ratio");
    out.lowEndRef = subRatio ? "analyzed: " + juce::String(*subRatio, 3) + " of spectral energy is 20-80 Hz"
                             : juce::String(juce::CharPointer_UTF8("not analyzed yet"));
    // Meter. The reference line carries the bar spread, because the spread is what says
    // whether to believe the meter at all -- above 1.5 the beats drifted and the number
    // above it is a guess dressed as a measurement (Meter.h).
    auto meterValue = database.jsonExtractDouble(rec.machine, "$.rhythm.meter");
    auto spread = database.jsonExtractDouble(rec.machine, "$.rhythm.meter_bar_spread");
    if (meterValue && *meterValue > 1)
    {
        out.meter = juce::String(static_cast<int>(*meterValue)) + " beats per bar";
        out.meterRef = spread
            ? "analyzed: bar spread " + juce::String(*spread, 3)
                  + juce::String(juce::CharPointer_UTF8("\xc3\x97"))
                  + (*spread > 1.5 ? juce::String(juce::CharPointer_UTF8(
                                          " \xe2\x80\x94 uneven, the beats drifted"))
                                   : juce::String(juce::CharPointer_UTF8(" \xe2\x80\x94 even")))
            : juce::String("analyzed");
    }
    else
    {
        out.meterRef = juce::String(juce::CharPointer_UTF8(
            "not measured \xe2\x80\x94 re-analyze with Groove on"));
    }

    auto fluxMean = database.jsonExtractDouble(rec.machine, "$.dsp.flux_mean");
    out.motionRef = fluxMean ? "analyzed: spectral flux " + juce::String(*fluxMean, 3)
                             : juce::String(juce::CharPointer_UTF8("not analyzed yet"));
...
void FileDetailsContent::reload()
{
    ...
    for (auto* row : { &bpmRow, &keyRow, &genreRow, &instrumentRow, &moodRow,
                        &grooveRow, &swingRow, &lowEndRow, &motionRow, &meterRow })
    ...
    else if (!isBatch())
    {
        ...
        setRowValue(grooveRow, fields.groove, fields.grooveRef);
        setRowValue(swingRow, fields.swing, fields.swingRef);
        setRowValue(lowEndRow, fields.lowEnd, fields.lowEndRef);
        setRowValue(motionRow, fields.motion, fields.motionRef);
        setRowValue(meterRow, fields.meter, fields.meterRef);
    }
    else
    {
        ...
        std::vector<juce::String> grooves, swings, lowEnds, motions, meters;
        for (const auto& r : records)
        {
            ...
            grooves.push_back(f.groove);
            swings.push_back(f.swing);
            lowEnds.push_back(f.lowEnd);
            motions.push_back(f.motion);
            meters.push_back(f.meter);
        }
        ...
        const std::pair<FieldRow*, const std::vector<juce::String>*> rows[] = {
            ...
            { &grooveRow, &grooves }, { &swingRow, &swings },
            { &lowEndRow, &lowEnds }, { &motionRow, &motions }, { &meterRow, &meters },
        };
...
// Renders human.$.generated as something readable rather than raw JSON. Absent on every
// file mira did not make, and the whole block hides in that case.
void FileDetailsContent::reloadGenerated()
{
    hasGenerated = false;
    juce::String out;
    ...
    if (!isBatch())
    {
        const auto human = juce::JSON::parse(juce::String(record.human));
        const auto gen = human.getProperty("generated", {});
        if (auto* g = gen.getDynamicObject())
        {
            hasGenerated = true;
            const auto line = [&out](const juce::String& k, const juce::String& v) {
                if (v.isNotEmpty()) out << k.paddedRight(' ', 9) << v << "\n";
            };
            line("prompt", g->getProperty("prompt").toString());
            line("avoid", g->getProperty("negative_prompt").toString());

            juce::StringArray settings;
            settings.add("seed " + g->getProperty("seed").toString());
            const auto cfg = static_cast<double>(g->getProperty("cfg"));
            settings.add("cfg " + juce::String(cfg, 1) + (cfg == 1.0 ? " (off)" : ""));
            settings.add(g->getProperty("steps").toString() + " steps");
            settings.add(g->getProperty("seconds").toString() + "s");
            line("settings", settings.joinIntoString("  -  "));

            if (auto* loras = g->getProperty("loras").getArray())
            {
                juce::StringArray rows;
                for (const auto& l : *loras)
                    if (auto* lo = l.getDynamicObject())
                    {
                        juce::String row = lo->getProperty("name").toString()
                                         + "  x" + juce::String(static_cast<double>(lo->getProperty("strength")), 2);
                        if (auto* gate = lo->getProperty("gate").getArray())
                            if (gate->size() == 2)
                                row << "  steps " << (*gate)[0].toString() << "-" << (*gate)[1].toString();
                        rows.add(row);
                    }
                // Subsequent LoRAs line up under the first rather than under the label.
                line("loras", rows.joinIntoString("\n" + juce::String().paddedRight(' ', 9)));
            }
            line("created", g->getProperty("created").toString());
        }
    }

    generatedLabel.setVisible(hasGenerated);
    generatedEditor.setVisible(hasGenerated);
    generatedEditor.setText(out.trimEnd(), juce::dontSendNotification);
}
...
void FileDetailsContent::save()
{
    ...
    // Single-value string fields, written the same way `key` is. An emptied editor is
    // NOT written -- that would store "" as a human override and permanently blank a
    // field the analyzer can still measure; Revert is the way back to pure machine
    // output, exactly as it is for every other row here.
    struct WordField { const FieldRow* row; const char* path; juce::String text; };
    const WordField wordFields[] = {
        { &grooveRow, "$.groove", grooveRow.editor.getText().trim() },
        { &swingRow, "$.swing", swingRow.editor.getText().trim() },
        { &lowEndRow, "$.low_end", lowEndRow.editor.getText().trim() },
        { &motionRow, "$.motion", motionRow.editor.getText().trim() },
    };
    ...
    if (isSegmentMode())
    {
        ...
        for (const auto& w : wordFields)
            if (changed(*w.row) && w.text.isNotEmpty())
                database.setSegmentHumanField(segmentId, w.path,
                                               juce::JSON::toString(juce::var(w.text), true).toStdString());
        reload();
        return;
    }
    ...
    for (const auto& r : records)
    {
        ...
        for (const auto& w : wordFields)
            if (changed(*w.row) && w.text.isNotEmpty())
                database.setHumanField(r.id, w.path,
                                        juce::JSON::toString(juce::var(w.text), true).toStdString());
    }
...
void FileDetailsContent::resized()
{
                        ...
                        &grooveRow, &swingRow, &lowEndRow, &motionRow, &meterRow })
    {
        if (row->hasVocab)
        {
            constexpr int pickerW = 150;
            row->sectionLabel.setBounds(margin, y, width - margin * 2 - pickerW - 8, 19);
            row->vocab.setBounds(width - margin - pickerW, y - 3, pickerW, 22);
        }
        else
        {
    ...
    if (!isBatch())
    {
        ...
        if (hasGenerated)
        {
            // Sized to the text. Counting newlines alone is not enough -- the prompt is
            // routinely 350+ characters and WRAPS, so a newline count clips it. Estimate
            // wrapped rows from the mono font's own width: at 11.5pt roughly 6.9px a
            // character, which is what makes the difference between a readable block and
            // a three-line window onto a paragraph.
            const int perRow = juce::jmax(20, static_cast<int>((innerWidth - 12) / 6.9));
            int rows = 0;
            for (const auto& l : juce::StringArray::fromLines(generatedEditor.getText()))
                rows += juce::jmax(1, (l.length() + perRow - 1) / perRow);
            const int lines = juce::jlimit(4, 20, rows + 1);
            generatedLabel.setBounds(margin, y, innerWidth, 19);
            y += 22;
            generatedEditor.setBounds(margin, y, innerWidth, lines * 18);
            y += lines * 18 + 18;
        }
